...
async def test_integration_get_contact(db_session: AsyncSession, contact_service_integration: ContactService):
    ...
    new_contact = Contact(id=uuid4(), full_name="Integration Contact", email="integration@example.com", message="Test message")
    await ContactRepository(db_session).create(new_contact)
...
async def test_integration_get_contacts_no_filter(db_session: AsyncSession, contact_service_integration: ContactService):
    await ContactRepository(db_session).create(Contact(id=uuid4(), full_name="Contact1", email="contact1@example.com", message="Msg1"))
    await ContactRepository(db_session).create(Contact(id=uuid4(), full_name="Contact2", email="contact2@example.com", message="Msg2"))
...
async def test_integration_get_contacts_filter_by_email(db_session: AsyncSession, contact_service_integration: ContactService):
    ...
    await ContactRepository(db_session).create(Contact(id=uuid4(), full_name="Filtered Contact", email=email_to_filter, message="Filtered Msg"))
    await ContactRepository(db_session).create(Contact(id=uuid4(), full_name="Other Contact", email="other@example.com", message="Other Msg"))
...
async def test_integration_get_contacts_filter_by_is_read(db_session: AsyncSession, contact_service_integration: ContactService):
    await ContactRepository(db_session).create(Contact(id=uuid4(), full_name="Read Contact", email="read@example.com", message="Read Msg", is_read=True))
    await ContactRepository(db_session).create(Contact(id=uuid4(), full_name="Unread Contact", email="unread@example.com", message="Unread Msg", is_read=False))
...
async def test_integration_create_contact(db_session: AsyncSession, contact_service_integration: ContactService):
    ...
    retrieved_contact = await ContactRepository(db_session).get(created_contact.id)
...
async def test_integration_update_contact(db_session: AsyncSession, contact_service_integration: ContactService):
    original_contact = Contact(id=uuid4(), full_name="Update Me", email="updateme@example.com", message="Original Msg")
    await ContactRepository(db_session).create(original_contact)

    updated_contact_data = Contact(id=original_contact.id, full_name="Updated Service Contact", email="updatedservice@example.com", message="Updated Msg", is_read=True)
    ...
    retrieved_contact = await ContactRepository(db_session).get(original_contact.id)
...
async def test_integration_delete_contact(db_session: AsyncSession, contact_service_integration: ContactService):
    contact_to_delete = Contact(id=uuid4(), full_name="Delete Me", email="deleteme@example.com", message="To be deleted")
    await ContactRepository(db_session).create(contact_to_delete)

    await contact_service_integration.delete_contact(contact_to_delete.id)
    
    # Verify it's deleted from the DB
    retrieved_contact = await ContactRepository(db_session).get(contact_to_delete.id)
...
async def test_integration_update_contact_message(db_session: AsyncSession, contact_service_integration: ContactService):
    ...
    await ContactRepository(db_session).create(obj_in=original_contact)
    ...
    retrieved_contact = await ContactRepository(db_session).get(id=contact_id)
